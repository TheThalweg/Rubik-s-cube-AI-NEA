...
public class Cube {
    ...
    //Both arrays which keep track of which stickers each edge/corner have
...
/*    //gets the edge index of a sticker given a side and its index within that side
    public int getEdgeIndex(int side, int index){
        if (side == 0){return (index+1)/2;}
        else if(side == 5){return ((index - 1)%8)/2+9;}
        else{switch (index){
            case 1 -> {
                return 6 - side;
            }
            case 7 -> {return 9 + (side - 2)%4;}
            default -> {return (((index+1)%8)/4+side)%4+4;}
        }}
    }*/
    //gets colour at given index for given side by shifting colour to the right and then bitwise anding with 0000000F
    ...
    //helper method to display the cube
    public void displayCube(){
        for(int side:sides) {
            System.out.println(Integer.toHexString(side));
        }
        System.out.println("-------------------------------------------------");
    }
