...
public class EnterNetController {
    ...
    // Checks whether the cube net contains all the required pieces
    private boolean containsPieces(){
        // Creates a list of all the pieces that need to be in the cube
        ArrayList<Integer> pieceTracker = new ArrayList<>();
        for(int piece:requiredPieces){
            pieceTracker.add(piece);
        }
        // Goes through all the edges in the cube net and checks removes the edge from the pieceTracker if it finds a piece that matches
        for(int[] edge: edgeIndexes){
            // Calculates for both orientations - if either of these orientations are in the pieceTracker than remove it
        ...
        // Does a similar process but with corners
        ...
        return pieceTracker.size() == 0; //if all the pieces have been accounted for pieceTracker will have a size of 0, therefore return true
    }
    ...
    private boolean checkPermutationParity(){
        ...
        for(int edgeIndex=0; edgeIndex<12 ;edgeIndex++){
            ...
            if(!visitedEdges.contains(edge)){
                ...
                do {
                    ...
                    /* Scans through list of required edges until a match is found. Once found the new edge to check is set as the edge found
                    in the position where the previous edge should have been found. This process repeats until we return to the original edge
                    index we started with at which point the parity tracker can be updated, and we can investigate a new cycle. We only check
                    the cycles that haven't been visited as kept track by the visitedEdges list.
                    */
                    for (int i = 0; i < 12; i++) {
        ...
        // Similar process but with the corners instead of edges
        ...
        return totalParity%2==0; // If total permutation parity is even, return true
    }
    ...
    // Is used to retrieve cube net from the GUI and represent it in our bitboard format
    @FXML
    public void retrieveNet() throws IOException {
        // Plays click sound
        AudioClip click = new AudioClip(Objects.requireNonNull(getClass().getResource("/click.mp3")).toExternalForm());
        click.play();
        // creates a 3x3 array called face array
        Paint[][] faceArray = new Paint[3][3];
        // For sticker in the yellow face add the colour of the sticker to the faceArray using the positions decided by their layout x and y coordinates
        for(Node sticker:yellow.getChildren()){
            Rectangle rectangleSticker = (Rectangle)sticker;
            faceArray[(int)(rectangleSticker.getLayoutX()/100)][(int)(rectangleSticker.getLayoutY()/100)] = rectangleSticker.getFill();
        }
        // Calls convertToCube and convertToSimplifiedCube to convert the faceArray into a valid bitboard
        convertToCube(0,faceArray,cube);
        convertToSimplifiedCube(0,faceArray,simplified);
        for(Node sticker:red.getChildren()){
            Rectangle rectangleSticker = (Rectangle)sticker;
            faceArray[(int)(rectangleSticker.getLayoutX()/100)][(int)(rectangleSticker.getLayoutY()/100)] = rectangleSticker.getFill();
        }
        // Same process repeats but for every face
        convertToCube(1,faceArray,cube);
        convertToSimplifiedCube(1,faceArray,simplified);
        for(Node sticker:green.getChildren()){
            Rectangle rectangleSticker = (Rectangle)sticker;
            faceArray[(int)(rectangleSticker.getLayoutX()/100)][(int)(rectangleSticker.getLayoutY()/100)] = rectangleSticker.getFill();
        }
        convertToCube(2,faceArray,cube);
        convertToSimplifiedCube(2,faceArray,simplified);
        for(Node sticker:orange.getChildren()){
            Rectangle rectangleSticker = (Rectangle)sticker;
            faceArray[(int)(rectangleSticker.getLayoutX()/100)][(int)(rectangleSticker.getLayoutY()/100)] = rectangleSticker.getFill();
        }
        convertToCube(3,faceArray,cube);
        convertToSimplifiedCube(3,faceArray,simplified);
        for(Node sticker:blue.getChildren()){
            Rectangle rectangleSticker = (Rectangle)sticker;
            faceArray[(int)(rectangleSticker.getLayoutX()/100)][(int)(rectangleSticker.getLayoutY()/100)] = rectangleSticker.getFill();
        }
        convertToCube(4,faceArray,cube);
        convertToSimplifiedCube(4,faceArray,simplified);
        for(Node sticker:white.getChildren()){
            Rectangle rectangleSticker = (Rectangle)sticker;
            faceArray[(int)(rectangleSticker.getLayoutX()/100)][(int)(rectangleSticker.getLayoutY()/100)] = rectangleSticker.getFill();
        }
        convertToCube(5,faceArray,cube);
        convertToSimplifiedCube(5,faceArray,simplified);
        // Checks whether cubeNet is valid. If it is then it loads the AI Menu
        if (validateCheck()){
            ...
            // Used to pass cube state to new menu
