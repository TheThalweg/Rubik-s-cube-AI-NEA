...
public class CFOP extends AICommon implements Runnable {
    ...
    public CFOP(Cube cube, Color[] colours, float duration, String[] solutions, CyclicBarrier barrier) {
        super(cube, colours, duration, solutions, barrier);
    }
    ...
    static String solve (Cube cube,int[][] cornerIndexes, int[][] edgeIndexes, Map<Short,String> orientationAlgs, Map<Short,String> permutationAlgs, int[][] sideOrders) {
        ...
        return cancelMoves(PLL(original,solution,permutationAlgs));
    ...
    public void run(){
        ...
        solutionContainer[0] = solution;
        try {
            barrier.await(); //signals to getSolution that the solve is complete
