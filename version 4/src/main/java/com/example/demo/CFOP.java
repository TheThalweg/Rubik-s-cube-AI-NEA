...
public class CFOP extends AICommon implements Runnable {
    ...
    private final Map<Short,String> orientationAlgs = Map.<Short, String>ofEntries(
            ...
            entry((short) 0b100100101101, "R U R' U' R' F R2 U R' U' F'"),
    ...
    static String findAndSolveEdges(Cube cube, String sequence, int[][] edgeIndexes, int cornerIndex, int cornerOrientation, int colourOne, int colourTwo){
        ...
        for(int[] edge: edgeIndexes){
            ...
            if (cube.getColour(edge[0]/8,edge[0]%8) == colourOne && cube.getColour(edge[1]/8,edge[1]%8) == colourTwo){
                System.out.println("edgeOne");
                System.out.println(cube.getColour(edge[0]/8,edge[0]%8));
                System.out.println(cube.getColour(edge[1]/8,edge[1]%8));
            ...
            for(int i:permutation){
                ...
                while (clone.getColour(5, 2 * (i-1)) != 6 || clone.getColour(i , 4) != 1 + i || clone.getColour((i % 4) + 1, 7) != (i % 4) + 2 || clone.getColour(i, 3) != 1 + i) {
                    ...
                    for (int[] corner : cornerIndexes) {
                        System.out.println("Corner index is: " + cornerIndex);
                        System.out.println(Arrays.toString(corner));
                        System.out.println(clone.getColour(corner[0] / 8, corner[0] % 8));
                        System.out.println(clone.getColour(corner[1] / 8, corner[1] % 8));
                        System.out.println(clone.getColour(corner[2] / 8, corner[2] % 8));
                    ...
                    //System.out.println(sequence);
                    //if(breakpoint>100){return sequence;}
                }
            }
    ...
    static String OLL(Cube cube, String sequence, Map<Short,String> orientationAlgs){
        System.out.println(Integer.toBinaryString(getOLLHash(cube)));
    ...
    static String PLL(Cube cube, String sequence, Map<Short,String> permutationAlgs){
        System.out.println(Integer.toBinaryString(getPLLHash(cube)));
    ...
    static String solve (Cube cube,int[][] cornerIndexes, int[][] edgeIndexes, Map<Short,String> orientationAlgs, Map<Short,String> permutationAlgs, int[][] sideOrders) {
        ...
        solution = OLL(original,cancelMoves(solution,false),orientationAlgs);
        return cancelMoves(PLL(original,solution,permutationAlgs),true);

    }

    @Override
    public void run(){
        String solution = solve(cube.clone(),cornerIndexes,edgeIndexes, orientationAlgs, permutationAlgs, sideOrders);
        System.out.println(solution);
        System.out.println(solution.split(" ").length);
        animateMoves(solution, cube, cubeGUI, duration, colours);
    }
