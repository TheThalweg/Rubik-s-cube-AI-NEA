...
/*    Cube cube;
    ...
    static String solveCorner(Cube cube, int whiteIndex, int colourOne, String sequence){
        ...
        if(whiteIndex<8){
            //cube.getColour(5,(6-(whiteIndex/8)))==6&&cube.getColour((8-whiteIndex)/2),4)==(8-whiteIndex)/2)+1)
            //i.e. if the corner below that is solved perform the algorithm that preserves that corner
            if(cube.getColour(5,(6-(whiteIndex/8)))==6&&cube.getColour((8-whiteIndex)/2,4)==(8-whiteIndex)/2+1){
                System.out.println("Corner preserved");
                ...
                System.out.println("Corner not preserved");
...
/*        String solution = whiteCross(cube, edgeIndexes,sideOrders);
        ...
        return cancelMoves(orientCorners(original,solution),true);
    }

    @Override
    public void run(){
        String solution = solve(cube.clone(),cornerIndexes,edgeIndexes, sideOrders);
